...
> 📚 **사전 지식**: 이 문서는 JavaScript의 기본 함수 문법과 객체/배열 접근을 이해한 개발자를 대상으로 합니다.  
> `function` 선언, `this` 개념, 객체/배열 속성 접근을 알고 있다면 충분합니다.
...
### 1.2 `this` 차이 — 화살표 vs. 전통 함수

> 🔥 **중급 주제**: `this` 바인딩은 JavaScript의 핵심 개념입니다.  
> 기본 함수 호출과 객체 메서드를 이해한 후 학습하는 것을 권장합니다.
...
- [ ] 실제 프로젝트에서 화살표 함수와 구조분해를 적절히 활용한다.
    
- [ ] 코드 리뷰에서 `this` 바인딩 관련 버그를 발견하고 수정할 수 있다.
...
- **04. 클래스 & Promise & async/await** - 객체 지향과 비동기 처리
- **02. var, const, let** - 변수 선언 (이전 주제)