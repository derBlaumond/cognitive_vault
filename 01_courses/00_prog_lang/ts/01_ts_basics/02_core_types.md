...
TypeScript는 JavaScript의 기본 타입을 그대로 확장한다.  
기본 타입들은 프로그램에서 가장 자주 쓰이며, TS의 정적 타입 시스템의 핵심 역할을 한다.
...
둘 다 “값이 없음”을 의미하지만 의미가 다름.

- **undefined**: 값이 할당되지 않음
    
- **null**: 명시적으로 값이 없음을 지정
    

TypeScript에서는 strictNullChecks 옵션 여부에 따라 동작이 달라진다.

---

### 6. symbol

ES6에서 추가된 유일(unique)한 값  
주로 식별자(identifier)로 사용됨.
...
Primitive Types는 JavaScript와 동일하게 보이지만,  
TypeScript에서는 **정적 타입 검사 덕분에 실수 예방 효과**가 매우 크다.  
특히 null/undefined를 엄격하게 처리함으로써 대규모 코드에서 오류를 줄일 수 있다.
...
TypeScript의 핵심은 강력한 **정적 타입 시스템**  
→ 코드 실행 전에 오류를 잡도록 설계됨.
