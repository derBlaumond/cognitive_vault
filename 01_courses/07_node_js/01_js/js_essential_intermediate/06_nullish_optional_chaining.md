...
> 📚 **사전 지식**: 이 문서는 JavaScript의 기본 연산자와 객체 속성 접근을 이해한 개발자를 대상으로 합니다.  
> 논리 연산자(`||`, `&&`), 객체 속성 접근, `null`/`undefined` 개념을 알고 있다면 충분합니다.
...
- [ ] 코드 리뷰에서 불필요한 `if` 체크를 옵셔널 체이닝으로 개선할 수 있다.
    
- [ ] 면접에서 `??`와 `||`의 차이를 설명할 수 있다.
...
- **01. Call Stack & Event Loop** - 실행 구조 이해
