# 클래스 & Promise & async/await

> 📚 **사전 지식**: 이 문서는 JavaScript의 객체, 함수, 프로토타입 개념을 이해한 개발자를 대상으로 합니다.  
> 객체 리터럴, 생성자 함수, 비동기 콜백의 기본 사용법을 알고 있다면 충분합니다.

---

## 1. 클래스(Class) — "프로토타입"을 읽기 쉽게 만드는 문법
...
## 2. Promise & async/await — 비동기의 표준

> 📚 **사전 지식**: 이 섹션은 비동기 콜백과 이벤트 루프 개념을 이해한 개발자를 대상으로 합니다.  
> `setTimeout`, 콜백 함수, 에러 처리의 기본 사용법을 알고 있다면 충분합니다.
...
### 2.4 병렬 vs. 순차 — 퍼포먼스 포인트

> 🔥 **중급 주제**: 성능 최적화를 위한 중요한 개념입니다.  
> 기본 Promise 사용법을 이해한 후 학습하는 것을 권장합니다.
...
### 2.5 마이크로태스크 우선순위(이벤트 루프와 연결)

> 🔥 **고급 주제**: 이벤트 루프의 내부 동작을 이해하고 싶은 개발자를 위한 선택적 내용입니다.  
> 기본 Promise와 async/await를 사용할 수 있다면 실무에 문제가 없습니다.
...
- [ ] 실제 프로젝트에서 Promise와 async/await를 적절히 활용한다.
    
- [ ] 코드 리뷰에서 비동기 처리 관련 버그를 발견하고 수정할 수 있다.
...
- **05. Map, Set, WeakMap, WeakSet** - 고급 컬렉션 타입
- **01. Call Stack & Event Loop** - 비동기 실행 구조 이해 (이전 주제)